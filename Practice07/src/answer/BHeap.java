...
public class BHeap<Key extends Comparable<Key>, Value> {
	private Entry_BinaryMinHeap[] a;      // a[0]은 사용 안함
	private int N;          // 힙의 크기, 즉, 힙에 있는 항목 수
	public BHeap(Entry_BinaryMinHeap[] harray, int initialSize) {  // 생성자  
		a = harray;
		N = initialSize;
	}
	...
	public void insert(Key newKey, Value newValue) { // 새로운 항목 삽입
		Entry_BinaryMinHeap temp = new Entry_BinaryMinHeap(newKey, newValue);	 // Entry 생성 
		a[++N] = temp;        // 새로운 키(항목)를 배열 마지막 원소 다음에 저장
		upheap(N);            // 위로 올라가며 힙속성 회복시키기 위해
	} 	
	/*
	public Entry_BinaryMinHeap deleteMin() {   // 최솟값 삭제
		Entry_BinaryMinHeap min = a[1];     // a[1]의 최솟값을 min으로 저장하여 리텬
		swap(1, N--);         // 힙의 마지막 항목과 교환하고 힙 크기 1 감소
		a[N+1] = null;        // 마지막 항목을 null로 처리
		downheap(1);          // 힙속성을 회복시키기 위해
		return min;
	}*/
	public Entry_BinaryMinHeap deleteMax() {   // 최솟값 삭제
		Entry_BinaryMinHeap max = a[1];     // a[1]의 최솟값을 min으로 저장하여 리텬
		swap(1, N--);         // 힙의 마지막 항목과 교환하고 힙 크기 1 감소
		a[N+1] = null;        // 마지막 항목을 null로 처리
		downheap(1);          // 힙속성을 회복시키기 위해
		return max;
	...
	private void downheap(int i) {   // i는 현재 노드의 인덱스
		while (2*i <= N) {           // i의 왼쪽 자식노드가 힙에 있으면
			...
			if (k < N && greater(k, k+1)) k++; //왼쪽과 오른쪽 자식의 승자를 결정하여 k가 승자의 인덱스가됨
	...
	private void swap(int i, int j) {      // a[i]와 a[j]를 교환
		Entry_BinaryMinHeap temp = a[i];   a[i] = a[j];   a[j] = temp; }	
	public void print(){  
		for (int i = 1; i < N+1; i++) {
			System.out.printf("["+a[i].getKey()+" "+a[i].getValue() + "] ");
		}
		System.out.println(); 	System.out.printf("힙 크기 = "+size()+"\n\n");
	}
}
