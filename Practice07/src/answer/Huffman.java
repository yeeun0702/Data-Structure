...
public class Huffman {
	...
	private Entry_Huffman[] a;      // a[0]은 사용 안함
	private int N;          // 힙의 크기

	public Huffman(Entry_Huffman[] harray, int initialSize) {  // 생성자  
		a = harray;
		N = initialSize;
	}
	...
	public Entry_Huffman createTree(){
		while (size() > 1){       // 힙에 1개의 노드만 남을 때까지
			Entry_Huffman e1 = deleteMin();  // 힙에서 최소 빈도수 가진 노드 제거하여 e1이 참조
			Entry_Huffman e2 = deleteMin();  // 힙에서 최소 빈도수 가진 노드 제거하여 e2이 참조
			Entry_Huffman temp = new Entry_Huffman(e1.getKey()+e2.getKey(),     // e1과 e2의 빈도수를 합산
	...
	public void preorder(Entry_Huffman node){ // 전위순회하며 허프만 코드를 만들고, 각 이파리 노드에서 코드 출력
	...
	public void insert(Entry_Huffman newEntry) {
		a[++N] = newEntry;        // 새로운 키를 배열 마지막 원소 다음에 저장
		upheap(N);                // 위로 올라가며 힙 속성 회복시키기 위해
	} 

	public Entry_Huffman deleteMin() {
		Entry_Huffman max = a[1];     // a[1]의 최대값을 max로
