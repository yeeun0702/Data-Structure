...
public class BTree {
	...
	private void splitNode(Node node) {
		...
		int middle = (node.count - 1) / 2;  // 수정: 선행자 올리도록 수정
	...
	private static boolean rotateRight2(Node rightNode) {
		...
		if (leftNode.isOverHalfFull() == false)
			return false;
		int lastData = leftNode.data[leftNode.count - 1];
	...
	private static boolean rotateLeft2(Node leftNode) {
		...
		if (index == parent.count)
			return false;
		Node rightNode = parent.child[index + 1];
		if (rightNode.isOverHalfFull() == false)
			return false;
		int firstData = rightNode.data[0];
	...
	private void remove(Node node, int value) {
		...
		if (0 < index && node.data[index - 1] == value) {
			if (node.isLeaf())
				...
				int value1 = getRightMostValue(node.child[index - 1]); // 수정: 가장 오른쪽 값 가져오기
				node.data[index - 1] = value1;    // 수정: 값을 가져온 값으로 대체
				remove(node.child[index - 1], value1);    // 수정: 대체한 값으로 재귀 호출
			}
		} else {
			Node childNode = node.child[index];
			if (childNode != null)
				remove(childNode, value);
		}
	}
	...
	private static int getRightMostValue(Node node) {
		if (node.child[node.count] == null)
			return node.data[node.count - 1];
		return getRightMostValue(node.child[node.count]);
	}
